...
public abstract class PoolSizeCalculator {
	...
	final static Logger LOG = LoggerFactory.getLogger(PoolSizeCalculator.class);
	/**
	...
	private void calculateOptimalCapacity(BigDecimal targetQueueSizeBytes) {
		...
		LOG.info("Target queue memory usage (bytes): " + targetQueueSizeBytes);
		LOG.info("createTask() produced " + creatTask().getClass().getName() + " which took " + mem
				+ " bytes in a queue");
		LOG.info("Formula: " + targetQueueSizeBytes + " / " + mem);
		LOG.info("* Recommended queue capacity (bytes): " + queueCapacity);
	}
	...
	private void calculateOptimalThreadCount(long cpu, long wait, BigDecimal targetUtilization) {
		...
		LOG.info("Target utilization: " + targetUtilization);
		LOG.info("Elapsed time (nanos): " + (testtime * 1000000));
		LOG.info("Compute time (nanos): " + cpu);
		LOG.info("Wait time (nanos): " + wait);
		LOG.info("Formula: " + numberOfCPU + " * " + targetUtilization + " * (1 + " + waitTime + " / "
				+ computeTime + ")");
		LOG.info("* Optimal thread count: " + optimalthreadcount);
	}
