...
*/
public class XiAnFang99HouseInfoWorker extends HtmlCommonWorker{
				...
				for(int i=1;i<trElements.size();i++){
					...
					for(int j=0;j<houseTdElements.size();j++){
						...
						for(Element houseElement:houseElements){
							...
							doingPage.getMetaMap().computeIfAbsent("houseType", mapKey->new ArrayList<>()).add(houseType);//房屋户型
							//少一个房屋用途
							doingPage.getMetaMap().computeIfAbsent("saleType", mapKey->new ArrayList<>()).add(saleType);//房屋类型（销售）
							doingPage.getMetaMap().computeIfAbsent("buildArea", mapKey->new ArrayList<>()).add(buildArea);//建筑面积
