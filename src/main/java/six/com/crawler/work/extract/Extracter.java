...
public class Extracter {
	...
	public ResultContext extract(Page page) {
		...
		if (null != paserItems) {
			for (int i = 0; i < paserItems.size(); i++) {
				ExtractItem doPaserItem = paserItems.get(i);
				List<String> doResults = extract(page, doPaserItem);
				resultContext.addResult(doPaserItem.getResultKey(), doResults);
			}
		}
		return resultContext;
	}

	private List<String> extract(Page page, ExtractItem paserItem) {
	...
	private String paserElement(FilterPath filterPath, String reslutAttName, Element element) {
		String result = "";
	 ...
	 * @return
	 */
	private List<String> extract(Page page, ExtractItem paserItem, ExtractPath path) {
	 ...
	 * @return
	 */
	public Elements getIndex(Elements elements, Selector temp) {
	 ...
	 */
	private void appendText(FilterPath filterPath, Node node, StringBuilder newBf) {
	...
	private static Elements findElements(Element element, Selector temp) {
	...
	private static Elements findElements(Elements elements, Selector temp) {
