 ...
 */
public class ShFangDiBuildingInfoWorker extends HtmlCommonWorker {

	RedisWorkQueue houseInfoQueue;
	

	public ShFangDiBuildingInfoWorker(String name, AbstractSchedulerManager manager, Job job, Site site,
			WorkQueue stored) {
	...
	public void onComplete(Page p) {
		List<String> houseInfoUrlList = p.getResultContext().takeResult("房间信息url_5");
		if(null!=houseInfoUrlList&&!houseInfoUrlList.isEmpty()){
			...
			for(int i=0;i<houseInfoUrlList.size();i++){
				...
				Page houseInfoPage = new Page(p.getSiteCode(), 1, houseInfoUrl, houseInfoUrl);
				...
				houseInfoPage.setReferer(p.getFinalUrl());
	...
	protected void insideInit() {
		houseInfoQueue = new RedisWorkQueue(getManager().getRedisManager(),
				"sh_fangdi_house_info");
